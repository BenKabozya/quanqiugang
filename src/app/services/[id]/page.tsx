...
export default function ServiceDetailPage() {
  ...
  async function submitComment() {
    ...
    await fetch(`/api/services/${id}/comments`, {
      method: "POST",
  ...
  async function likeComment(commentId: string) {
    ...
    await fetch(`/api/service-comments/${commentId}/like`, { method: "POST" });
    load();
  }
  ...
  const visibleComments = showAllComments ? service.comments : service.comments.slice(0, 2);
  const hiddenCount = service.comments.length - visibleComments.length;
        ...
        <div className="bg-panel border border-line rounded-2xl overflow-hidden mt-4 w-full">
          {service.imageUrl && (
            <div className="w-full bg-white/5">
              <img src={service.imageUrl} alt={service.title} className="w-full h-auto block" />
            </div>
          )}
          {service.videoUrl && (
          ...
          {service.mediaCaption && (
            <p className="text-white/40 text-xs italic px-4 sm:px-5 pt-3">{service.mediaCaption}</p>
          )}

          <div className="p-4 sm:p-6 lg:p-8 xl:p-10">
            <p className="text-white/40 text-xs mb-2">Service · by {service.admin.name}</p>
